-- admin_health_report: "tiene su negocio" ahora mira sus productos (tabla products), ya no la
-- ficha única de business_profile.
    ...
    'customers', coalesce((
      select jsonb_agg(t order by t.ends) from (
        select u.email, s.status, s.current_period_end as ends,
               ...
               exists (select 1 from public.products p where p.user_id = s.user_id and coalesce(p.product, '') <> '') as has_business,
