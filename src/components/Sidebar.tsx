import { LayoutDashboard, Megaphone, Users, ImageIcon, BarChart2, Settings, Zap, ChevronDown, Bell } from "lucide-react";
import { useState } from "react";

const navItems = [
  { icon: LayoutDashboard, label: "Dashboard", active: true },
  { icon: Megaphone, label: "Campañas", badge: "12" },
    ...
    <aside className="flex flex-col w-64 min-h-screen border-r border-sidebar-border bg-sidebar animate-slide-in-left" style={{ background: "var(--gradient-sidebar)" }}>
      ...
      <div className="px-4 py-3 border-b border-sidebar-border">
        <button className="flex items-center justify-between w-full px-3 py-2 rounded-lg hover:bg-sidebar-accent transition-colors group">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded gradient-brand flex items-center justify-center text-xs font-bold text-primary-foreground">A</div>
            <span className="text-sm font-medium text-sidebar-foreground group-hover:text-foreground transition-colors">Acme Corp</span>
          </div>
      ...
      <div className="px-4 py-4 border-t border-sidebar-border">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-full gradient-brand flex items-center justify-center text-sm font-bold text-primary-foreground">J</div>
          <div className="flex-1 min-w-0">
            <div className="text-sm font-medium text-foreground truncate">Juan García</div>
