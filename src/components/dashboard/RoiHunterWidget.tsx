  ...
  return (
    <section className="card-surface rounded-2xl p-6">
      <div className="flex items-start justify-between gap-3 flex-wrap mb-4">
        <div>
          <div className="text-[10px] uppercase tracking-[0.22em] text-muted-foreground font-medium mb-1">Ofertas que sigues</div>
          <h3 className="font-display font-semibold text-lg text-foreground flex items-center gap-2">
            <Crosshair className="w-4 h-4 text-primary" strokeWidth={1.8} /> Cazador de ofertas
          </h3>
        </div>
        {rows.length > 0 && (
          <div className="flex gap-2 text-[11px]">
            <span className="rounded-full px-2.5 py-1 bg-success/15 text-success font-semibold">↑ {up} con más anuncios</span>
            <span className="rounded-full px-2.5 py-1 bg-secondary text-muted-foreground font-semibold">= {rows.length - up - down} sin cambios</span>
            {down > 0 && <span className="rounded-full px-2.5 py-1 bg-destructive/10 text-destructive font-semibold">↓ {down} con menos anuncios</span>}
          </div>
        )}
      </div>
      ...
      {rows.length === 0 ? (
        <div className="text-sm text-muted-foreground">
          Sigue las ofertas que te interesan y aquí verás, semana a semana, cuáles ponen más anuncios (buena señal: les está funcionando)
          y cuáles los apagan. Seguir una oferta cuesta {CREDIT_COSTS.follow_offer} créditos; ver cómo cambia es gratis.
          <div className="mt-4">
            <button onClick={() => onNavigate?.("Ofertas")} className="btn-primary-nova px-4 py-2 rounded-lg text-[13px] inline-flex items-center gap-1.5">
              Ver ofertas para seguir <ArrowRight className="w-3.5 h-3.5" />
            </button>
          </div>
        </div>
      ) : (
        <>
              ...
              <div key={r.offer.id} className="py-2.5 flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[11px] text-muted-foreground truncate">
                    {MARKET_NAME[r.offer.market] ?? r.offer.market} · {r.offer.active_ads} anuncios activos · puntaje {r.offer.winner_score}/100
                    {r.days_tracked > 0 ? ` · ${r.days_tracked} días de seguimiento` : " · seguimiento desde hoy"}
                  </div>
                </div>
                <Delta value={r.ads_delta} suffix="anuncios" />
              </div>
            ))}
          </div>
          <button onClick={goFollowing} className="mt-3 text-[12px] text-muted-foreground hover:text-foreground transition-colors flex items-center gap-1">
            Ver las {rows.length} ofertas que sigues <ArrowRight className="w-3 h-3" />
          </button>
        </>
      )}
    </section>
