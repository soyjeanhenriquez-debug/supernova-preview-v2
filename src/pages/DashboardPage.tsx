...
export function DashboardPage({ onNavigate }: Props) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="page-heading font-display text-2xl text-foreground">INTELLIGENCE HUB</h2>
        <p className="text-sm text-muted-foreground mt-3">Tu motor de descubrimiento de ofertas ganadoras</p>
      </div>
        ...
        {recent.length === 0 ? (
          ...
          <div className="grid md:grid-cols-3 gap-3">
            {recent.map((p) => {
              ...
              return (
                <div key={p.id} className="card-surface rounded-xl p-3 ad-card-hover">
                  <ProjectThumb seed={p.name} className="mb-3" />
      ...
      <div className="card-surface rounded-xl p-5">
        <div className="flex items-center justify-between mb-2">
          <div>
            ...
            <div className="text-xs text-muted-foreground">Se renuevan en {renewal.days} días ({renewal.formatted})</div>
          </div>
        ...
        <div className="text-xs text-muted-foreground mt-2"><CountUp value={balance} /> de <CountUp value={limit} /> disponibles</div>
      </div>
    </div>
  );
}
...
function StatCard({ label, value, icon, highlight }: { label: string; value: number; icon: React.ReactNode; highlight?: "primary" | "destructive" }) {
  ...
  return (
    <div className="card-surface rounded-xl p-5 ad-card-hover">
      ...
      <div className={`text-3xl font-display font-extrabold mt-2 ${highlight === "destructive" ? "text-destructive" : "text-foreground"}`}>
        <CountUp value={value} />
      </div>
    </div>
  );
...
function ActionCard({ title, subtitle, onClick }: { title: string; subtitle: string; onClick: () => void }) {
  return (
    <button onClick={onClick} className="card-surface rounded-xl p-6 text-left ad-card-hover group">
